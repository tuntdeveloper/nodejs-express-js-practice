...
import { ProductModel } from '../../.history/src/db/product_20240412131038';
...
export const addProductToCart =  async (req: express.Request, res: express.Response) =>  {
    execute(
        ...
        async (userId) => {
            ...
            let products = existCart.products.map(value => {
                if((value as typeof ProductModel)._id === existProduct._id)
