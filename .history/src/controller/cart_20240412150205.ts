...
export const addProductToCart =  async (req: express.Request, res: express.Response) =>  {
    execute(
        ...
        async (userId) => {
            ...
            // let products = existCart.products.map<typeof ProductModel>(function(value, index) {
            ...
            const products: any[] = [];
             ...
             existCart.set({products: products});

            const result = existCart.save().then((value) => value.toObject);

            return res.status(200).json(existCart).end();
        }
    );
}
