...
export const cartAddProduct = async (req: express.Request, res: express.Response) => {
    execute(
        ...
        async (userId) => {
            ...
            const existTemptProduct = await getProductById(productId);

            if (!existTemptProduct) {
            ...
            const  existProduct : typeof ProductModel = {...existTemptProduct,  quantity: Number(quantity), amount: existTemptProduct.price * Number(quantity)};
            ...
            if(existCart.products.some(item => item.id == existProduct.id)) {
            ...
            const result = await existCart.set({
                products: [...existCart.products, {...existProduct,quantity: Number(quantity), amount: existProduct.price * Number(quantity) }]
            }).save();
...
export const cartDeleteProductById = async (req: express.Request, res: express.Response) => {
    execute(
        req,
        res,
        async (userId) => {
