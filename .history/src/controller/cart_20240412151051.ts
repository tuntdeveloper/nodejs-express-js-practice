...
export const addProductToCart = async (req: express.Request, res: express.Response) => {
    execute(
        ...
        async (userId) => {
            ...
            let products = existCart.products.map<typeof ProductModel>(function (value, _) {
                if (value.id === existProduct.id) {
                    let newValuåe = { ...value, quantity: Number(quantity), amount: existProduct.price * Number(quantity), };
                    return newValue;
                }
